...
function reset() {
  new Audio("assets/sound/514160__edwardszakal__score-beep.mp3").play();
  clearInterval(ID);
  Vx = -1;
  Vy = -1;
    ...
    setInterval(() => {
      if (marginLeft(ball) < 0) {
        document.querySelector("#comp-score").innerHTML =
          Number(document.querySelector("#comp-score").innerHTML) + 1;
        reset();
        return;
      }
      if (marginLeft(ball) + 20 > 1088) {
        document.querySelector("#user-score").innerHTML =
          Number(document.querySelector("#user-score").innerHTML) + 1;
        reset();
        return;
      }
      ...
      if (marginTop(ball) < 0 || marginTop(ball) + 20 > 544) {
        new Audio(
          "assets/sound/270332__littlerobotsoundfactory__hit-03.wav"
        ).play();
        Vy = -Vy;
      }
      ...
      if (collisionDetected(paddle)) {
        new Audio(
          "assets/sound/361299__mabdurrahman__returnracquet.wav"
        ).play();
        let angle;
      ...
      compPaddle.style.marginTop = `${
        marginTop(compPaddle) +
        (ball.centerY - (marginTop(compPaddle) + 36)) * compLevel
      }px`;
